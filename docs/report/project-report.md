...
完成的学生/助手文字轮次按课程有界保存，刷新后恢复；欢迎语、图片和未完成半轮不保存。机器人页另提供百度 ASR/TTS 链路，限制：不是实时双工；TTS 质量取决于浏览器和操作系统；历史不做账号或跨设备同步。
...
- Web BFF 清洗 Core 设备响应，不把管理令牌或完整状态发送浏览器；设备命令另有固定白名单代理，浏览器只可发受限参数。
