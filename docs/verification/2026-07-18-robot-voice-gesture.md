...
- 浏览器端使用 MediaPipe Hand Landmarker WASM/CPU 处理本地摄像头帧；张手移动网页光标，握拳保持 1.2 秒后触发一次页面内按钮点击，并在设备在线时发送受限的 XTest 鼠标命令。
...
- 用户提供的 `XIAN-HHappy/handpose_x` 是 MIT 许可的 21 点关键点模型代码，仓库不含手部检测器，预训练权重通过外部网盘提供；它可作为后续 ONNX/NB 候选，当前不替换已验证的浏览器模型。
